...
var authenticateRequest = (opts) => {
  ...
  const isSatellite = (0, import_utils.handleValueOrFn)(options?.isSatellite, clerkRequest.clerkUrl, env.isSatellite);
  const domain = (0, import_utils.handleValueOrFn)(options?.domain, clerkRequest.clerkUrl) || env.domain;
  const signInUrl = options?.signInUrl || env.signInUrl;
  const proxyUrl = absoluteProxyUrl(
    (0, import_utils.handleValueOrFn)(options?.proxyUrl, clerkRequest.clerkUrl, env.proxyUrl),
...
var clerkClient = new Proxy(clerkClientSingleton, {
  get(_target, property) {
    ...
    const env = { ...loadApiEnv(), ...loadClientEnv() };
    if (env.secretKey) {
      clerkClientSingleton = createClerkClient({ ...env, userAgent: `${"@clerk/clerk-sdk-node"}@${"5.0.76"}` });
      return clerkClientSingleton[property];
    }
    const c = createClerkClient({ ...env, userAgent: `${"@clerk/clerk-sdk-node"}@${"5.0.76"}` });
    return c[property];
  },
  set() {
    return false;
